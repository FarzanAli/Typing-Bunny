...
import AccountLoginIcon from './components/account/accountLoginIcon';
...
export default class Main extends Component {
  ...
  async sendData() {
    var score = {}
    var d = new window.Date().toISOString().slice(0, 19).replace('T', ' ');
    score["username"] = this.state.credentials.username
    score["email"] = this.state.credentials.email
    score["timeStamp"] = d
  ...
  startTimer() {
    ...
    let timer = setInterval(() => {
      ...
      if (this.state.runTimer === false) {
        if (this.state.loggedIn && this.state.input.length === this.state.typingText.length) {
          this.sendData()
        }
        clearInterval(timer);
      }
      ...
      <>
        <Router>
          {/* {this.state.showNotification && 
            ...
            setNotificationData = {this.setNotificationData.bind(this)}
            />
          } */}
